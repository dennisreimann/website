...
The `user` value is handled in the case statement contained in the `isSignedIn` function: If it is `Nothing` the case statement returns `False`. In case the `user` is signed in the statement destructures the `Just` value to `currentUser` and returns `True`.

[Destructuring](elm-data-structures-record-tuple.html#destructuring) is a really nice feature in Elm and case statements can destructure all kinds of data structures. We can leverage this to not just return a `Boolean` but the actual login:
...
Destructuring in case statements is a very powerful feature and you will find it in many places in Elm code: Update functions use destructuring for `Msg` types and the `Result` and `Task` types are handled very similar to how we deal with `Maybe`.
