...
const isEnglish = (filePath) =>
  (context) => {
    if (filePath && filePath.match(/^pages\/(articles|contact|privacy|projects|home|pairing|donate|thankyou)/)) {
      return true
...
export default {
  createHelper (file, isDev, siteHost, assetHost, defaultScheme) {
    ...
    const assetUrl = (filePath, scheme) => {
      const hostPart = assetHost ? `${scheme ? scheme + ':' : ''}//${assetHost}` : ''
    ...
    return {
      ...
      nav: {
        isHome: streamFile.match(/^pages\/(home|index)/),
